...
const RolForm = () => {
  ...
  const fetchRoles = async () => {
    try {
      const res = await axios.get("http://ambitious-forest-0ecbd371e.6.azurestaticapps.net/destinix/api/Rol/RolController.php");
      setRoles(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editando) {
        await axios.post("http://ambitious-forest-0ecbd371e.6.azurestaticapps.net/destinix/api/Rol/RolController.php?editar=1", {
          ...form,
          idRol
        });
        alert("Rol actualizado");
      } else {
        await axios.post("http://ambitious-forest-0ecbd371e.6.azurestaticapps.net/destinix/api/Rol/RolController.php", form);
  ...
  const handleEliminar = async (id) => {
    if (window.confirm("¿Deseas eliminar este rol?")) {
      try {
        await axios.delete(`http://ambitious-forest-0ecbd371e.6.azurestaticapps.net/destinix/api/Rol/RolController.php?id=${id}`);
        fetchRoles();
      } catch (error) {
