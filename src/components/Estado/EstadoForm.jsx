...
const EstadoForm = () => {
  ...
  const fetchEstados = async () => {
    try {
      const response = await axios.get("https://destinixweb-h7cxddbtb0duddbv.brazilsouth-01.azurewebsites.net/destinix/api/Estado/EstadoController.php");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editando) {
        ...
        await axios.post("https://destinixweb-h7cxddbtb0duddbv.brazilsouth-01.azurewebsites.net/destinix/api/Estado/EstadoController.php?editar=1", formData);
        alert("Estado actualizado correctamente");
      } else {
        await axios.post("https://destinixweb-h7cxddbtb0duddbv.brazilsouth-01.azurewebsites.net/destinix/api/Estado/EstadoController.php", formData);
  ...
  const handleEliminar = async (id) => {
    if (window.confirm("¿Está seguro de eliminar este estado?")) {
      try {
        await axios.delete(`https://destinixweb-h7cxddbtb0duddbv.brazilsouth-01.azurewebsites.net/destinix/api/Estado/EstadoController.php?id=${id}`);
        fetchEstados();
      } catch (error) {
