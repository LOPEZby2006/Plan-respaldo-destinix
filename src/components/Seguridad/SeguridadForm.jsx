...
const SeguridadForm = () => {
  ...
  const fetchSeguridad = async () => {
    try {
      const res = await axios.get("http://ambitious-forest-0ecbd371e.6.azurestaticapps.net/destinix/api/Seguridad/SeguridadController.php");
      setDatos(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editando) {
        await axios.post("http://ambitious-forest-0ecbd371e.6.azurestaticapps.net/destinix/api/Seguridad/SeguridadController.php?editar=1", {
          ...form,
          id_seguridad
        });
        alert("Registro actualizado");
      } else {
        await axios.post("http://ambitious-forest-0ecbd371e.6.azurestaticapps.net/destinix/api/Seguridad/SeguridadController.php", form);
  ...
  const handleEliminar = async (id) => {
    if (window.confirm("¿Deseas eliminar este registro?")) {
      try {
        await axios.delete(`http://ambitious-forest-0ecbd371e.6.azurestaticapps.net/destinix/api/Seguridad/SeguridadController.php?id=${id}`);
        fetchSeguridad();
      } catch (error) {
