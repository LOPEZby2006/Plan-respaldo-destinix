...
class PHPMailer
{
     ...
     * 'ambitious-forest-0ecbd371e.6.azurestaticapps.net.localdomain'.
     *
     * @see PHPMailer::$Helo
     *
     * @var string
     ...
     * @var string
     */
    public $Host = 'ambitious-forest-0ecbd371e.6.azurestaticapps.net';
    ...
    /**
     * The SMTP HELO/EHLO name used for the SMTP conexionection.
    ...
    /**
     * What kind of encryption to use on the SMTP conexionection.
    ...
    /**
     * Options array passed to stream_context_create when conexionecting via SMTP.
     *
     * @var array
     */
    public $SMTPOptions = [];
     ...
     * @see SMTP::DEBUG_conexionECTION: As SERVER plus conexionection status
    ...
    /**
     * Whether to keep the SMTP conexionection open after each message.
     * If this is set to true then the conexionection will remain open after a send,
     * and closing the conexionection will require an explicit call to smtpClose().
    ...
    {
        //Close any open SMTP conexionection nicely
        $this->smtpClose();
    }
    ...
    public function postSend()
    {
        try {
            ...
            if ($this->Mailer === 'smtp' && $this->SMTPKeepAlive == true && $this->smtp->conexionected()) {
                $this->smtp->reset();
            }
            if ($this->exceptions) {
                throw $exc;
    ...
    protected function smtpSend($header, $body)
    {
        $header = static::stripTrailingWSP($header) . static::$LE . static::$LE;
        $bad_rcpt = [];
        if (!$this->smtpconexionect($this->SMTPOptions)) {
            throw new Exception($this->lang('smtp_conexionect_failed'), self::STOP_CRITICAL);
        }
    ...
    /**
     * Initiate a conexionection to an SMTP server.
     ...
     * @return bool
     */
    public function smtpconexionect($options = null)
    {
        if (null === $this->smtp) {
        ...
        //Already conexionected?
        if ($this->smtp->conexionected()) {
            return true;
        }
        ...
        if ($this->Host === null) {
            $this->Host = 'ambitious-forest-0ecbd371e.6.azurestaticapps.net';
        }
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ($this->smtp->conexionect($prefix . $host, $port, $this->Timeout, $options)) {
                try {
                    if ($this->Helo) {
                        $hello = $this->Helo;
                    } else {
                    ...
                    //* it's not disabled
                    //* we are not conexionecting to ambitious-forest-0ecbd371e.6.azurestaticapps.net
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            $message = $this->getSmtpErrorMessage('conexionect_host');
                    ...
                    $lastexception = $exc;
                    $this->edebug($exc->getMessage());
                    //We must have conexionected, but then failed TLS or Auth, so close conexionection nicely
                    $this->smtp->quit();
                }
            }
        }
        //If we get here, all conexionection attempts have failed, so close conexionection hard
        $this->smtp->close();
        ...
        if ($this->exceptions) {
            // no exception was thrown, likely $this->smtp->conexionect() failed
            $message = $this->getSmtpErrorMessage('conexionect_host');
            throw new Exception($message);
        }
    ...
    {
        if ((null !== $this->smtp) && $this->smtp->conexionected()) {
            $this->smtp->quit();
            $this->smtp->close();
        }
    }
    ...
    public function setLanguage($langcode = 'en', $lang_path = '')
    {
        ...
        $PHPMAILER_LANG = [
            ...
            'conexionect_host' => 'SMTP Error: Could not conexionect to SMTP host.',
            ...
            'smtp_conexionect_failed' => 'SMTP conexionect() failed.',
            'smtp_detail' => 'Detail: ',
     ...
     * Returns 'ambitious-forest-0ecbd371e.6.azurestaticapps.net.localdomain' if unknown.
     *
     * @return string
     */
    protected function serverHostname()
    {
        ...
        if (!static::isValidHost($result)) {
            return 'ambitious-forest-0ecbd371e.6.azurestaticapps.net.localdomain';
        }
    ...
    protected function lang($key)
    {
        ...
        if (array_key_exists($key, $this->language)) {
            if ('smtp_conexionect_failed' === $key) {
                //Include a link to troubleshooting docs on SMTP conexionection failure.
